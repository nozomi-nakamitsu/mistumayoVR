...
<div>
 ...
 {{peer?peer.options.user.name:""}}
    ...
    <input type="text" id="js-room-name-input" /> 
    <button id="js-room-join-button">join</button>
</div>
...
// SkyWay
 const remoteVideos = document.getElementById('js-remote-streams');
  ...
  // VRM Settings
  loader.load("/resource/sample2.vrm",
    async (gltf) => {
  ...
  // SkyWay(WebRTC)
  console.log(API_KEY)
  console.log("test",process.env.SKY_WAY_API_KEY)
  ...
  peer.value.options.user.name=document.getElementById('js-room-name-input').value.trim()
  console.log("peer1.value",peer.value)
      ...
      const room = peer.value.joinRoom(roomName, {
        mode: 'sfu',
        stream
      })
      // room.on('open', () => {
